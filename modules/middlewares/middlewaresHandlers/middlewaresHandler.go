...
import (
	...
	"github.com/nanthaphol2/bigdogshop/pkg/auth"
)
...
const (
	...
	roleErr        middlewareHandlersErrCode = "middlware-006"
)
...
func (h *middlewaresHandler) Authorize(expectRoleId int) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		rolesUser, errfindid := h.middlewaresUsecase.FindOneUserById(claims.Id)
		if errfindid != nil {
			return entities.NewResponse(c).Error(
				fiber.ErrUnauthorized.Code,
				string(roleErr),
		...
		if rolesUser.RoleId != expectRoleId {
			return entities.NewResponse(c).Error(
				fiber.ErrUnauthorized.Code,
				string(authorizeErr),
				"no permission to access",
